//#!本文件由share.js自动产生, 命令行为: node share.js gen media CRUD ..
'use strict';
/**
 * media数据库访问类
 */
...
// 创建索引，以及设置唯一键
// TODO: 根据实际结构确定索引结构
media.index({ media_name : 1 })
    .index({ category : 1, media_name : 1}, { unique : true })
    .index({tags : 1});
...
if(isme(__filename)){
  if(process.argv.length > 2 && isme(__filename)){
    ...
    console.log('mediadb.js '+ testcmd.join('|'));
  }
}
